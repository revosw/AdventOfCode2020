...
public class day4 {
    public static void main(String[] args) {
        ...
        var result = Arrays.stream(passports)
                .map(s -> s.replace("\n", " "))
                .filter(validByr)
                .filter(validEcl)
                .filter(validEyr)
    ...
    public static Predicate<String>
            validByr = s -> s.matches(".*byr:(19[2-9][0-9]|200[0-2]).*"),
            validIyr = s -> s.matches(".*iyr:20(1[0-9]|20).*"),
            validEyr = s -> s.matches(".*eyr:20(2[0-9]|30).*"),
            validHgt = s -> s.matches(".*hgt:((1([5-8][0-9]|9[0-3])cm)|((59|6[0-9]|7[0-6])in)).*"),
            validHcl = s -> s.matches(".*hcl:#[0-9a-f]{6}\\b.*"),
            validEcl = s -> s.matches(".*ecl:(amb|blu|brn|gry|grn|hzl|oth).*"),
            validPid = s -> s.matches(".*pid:\\d{9}\\b.*");

    public static final String DATA = "ecl:#eef340 eyr:2023 hcl:#c0946f pid:244684338 iyr:2020 cid:57 byr:1969 hgt:152cm\n" +
            "\n" +
